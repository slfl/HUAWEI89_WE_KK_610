...
my $total_part_number_iter = 0;
    ...
    while ($iter < $TotalCustemChips)
    {
        ...
        if ($DEV_TYPE1[$iter] eq "00")      
        {
            ...
            {
                  printf "die here more than 1 Discrete DDR used!\n";
            }
        }
...
sub DeviceListParser_LPSDRAM
{
    ...
    $DEV_Model[$CustCS_id] = '"' . $PartNum . '"';
    print "\nCustCS ID num is $CustCS_id, part number:$CustCS_PART_NUMBER[$CustCS_id],DEV_Model[$CustCS_id]:$DEV_Model[$CustCS_id]\n" ;
    ...
    while ($iter<$num_part_found)
    {
        ...
        $DEV_Vendor[$id]='"'.$VENDOR[$id].'"';

		printf "DEV_Vendor is $DEV_Vendor[$id] \n ";
